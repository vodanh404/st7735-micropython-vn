...
class TFT(object):
    ...
    FOREST = TFTColor(0x00, 0x80, 0x00)
    ...
    AQUA = TFTColor(0x00, 0xFF, 0xFF)
    FUCHSIA = TFTColor(0xFF, 0x00, 0xFF)
    ORANGE = TFTColor(0xFF, 0xA5, 0x00)
    BROWN = TFTColor(0xA5, 0x2A, 0x2A)
    ...
    # =================== Vẽ 1 Ký Tự ===================
    def char(self, aPos, aChar, aColor, aFont, aSizes=(1, 1), aBgColor=None):
        if aFont is None or ord(aChar) not in aFont:
            print(f"Error: Char '{aChar}' (ord: {ord(aChar)}) not found in font")
            return

        char_data = aFont[ord(aChar)]
        bitmap = char_data.get('bitmap', [])
        font_width = char_data.get('width', len(bitmap))
        font_height = 20  # Updated to match new BITMAP_HEIGHT

        if not bitmap:
            print(f"Error: Empty bitmap for char '{aChar}' (ord: {ord(aChar)})")
            return

        if aBgColor is not None:
            self.fillrect(aPos, (font_width * aSizes[0], font_height * aSizes[1]), aBgColor)

        x_pos, y_pos = aPos
        for col in range(len(bitmap)):
            pixel_data = bitmap[col]
            for row in range(font_height):
                if (pixel_data >> row) & 1:
                    px = x_pos + col * aSizes[0]
                    py = y_pos + row * aSizes[1]
                    self.fillrect((px, py), aSizes, aColor)

    # =================== Vẽ 1 Chuỗi ===================
    def text(self, aPos, aString, aColor, aFont, aSizes=(1,1), nowrap=False, aBgColor=None):
        if aFont is None:
            print("Error: Font is None")
            return
    
        fonth = 20  # Updated to match new BITMAP_HEIGHT
        startx, starty = aPos
        x, y = startx, starty
        ...
        for ch in aString:
            ...
            char_data = aFont.get(ord(ch))
            if char_data:
                char_width = char_data.get('width', len(char_data.get('bitmap', [])))
                self.char((x, y), ch, aColor, aFont, aSizes, aBgColor)
                spacing = punctuation_spacing if ch in punctuation else char_spacing
                x += (char_width + spacing) * aSizes[0]
            else:
                print(f"Error: Char '{ch}' (ord: {ord(ch)}) skipped, not in font")

            if not nowrap and x + char_width * aSizes[0] > self._size[0]:
                x = startx
                y += fonth * aSizes[1] + 2
